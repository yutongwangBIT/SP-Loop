...
class Utility
{
  public:
    ...
    static void CvMatToMsgArrayFixedSize(cv::Mat &InMat, TMes &MsgArray)
    {
        ...
        {
            for(size_t idx=0;idx<MsgArray.size();++idx) MsgArray[idx] = static_cast<TMat>(0.0);
        }
        else if(InMat.cols == 1) //1D
        {
            for(size_t idx=0;idx<MsgArray.size();++idx) MsgArray[idx] = InMat.at<TMat>(idx);
        }
        else
        {
    ...
    static void MsgArrayFixedSizeToCvMat(cv::Mat &InMat, TMes &MsgArray)
    {
        if(InMat.cols == 1) //1D
        {
            for(size_t idx=0;idx<MsgArray.size();++idx) InMat.at<TMat>(idx) = MsgArray[idx];
        }
        else
        {
            for(int idx=0;idx<InMat.rows;++idx)

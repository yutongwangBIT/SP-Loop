...
void PoseGraphSP::registerPub(ros::NodeHandle &n)
{
...
void PoseGraphSP::loadVocabulary(std::string voc_path)
{
...
void PoseGraphSP::addKeyFrameSP(KeyFrameSP* cur_kf, bool flag_detect_loop)
{
    //shift to base frame
    Vector3d vio_P_cur;
    Matrix3d vio_R_cur;
       ...
       /* if (cur_kf->findConnection(old_kf))
        {
            ...
            {  
                ...
                vio_R_cur = w_r_vio *  vio_R_cur;
                cur_kf->updateVioPose(vio_P_cur, vio_R_cur);
                list<KeyFrameSP*>::iterator it = keyframelist.begin();
    ...
    if (SHOW_S_EDGE)
    {
        list<KeyFrameSP*>::reverse_iterator rit = keyframelist.rbegin();
        for (int i = 0; i < 4; i++)
        {
...
KeyFrameSP* PoseGraphSP::getKeyFrameSP(int index)
{
...
int PoseGraphSP::detectLoop(KeyFrameSP* keyframe, int frame_index)
{
  ...
  //  QueryResults ret;
    ...
    TicToc t_add;
    db.add(keyframe->descriptors_converted); 
...
void PoseGraphSP::addKeyFrameSPIntoVoc(KeyFrameSP* keyframe)
{
    // put image into image_pool; for visualization
    cv::Mat compressed_image;
    if (DEBUG_IMAGE)
...
void PoseGraphSP::optimize4DoF()
{
    while(true)
    {
        ...
        if (cur_index != -1)
        {
            ...
            TicToc tmp_t;
            m_keyframelist.lock();
            KeyFrameSP* cur_kf = getKeyFrameSP(cur_index);
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                if((*it)->has_loop)
                {
                    ...
                    int connected_index = getKeyFrameSP((*it)->loop_index)->local_index;
...
//        printf("path p: %f, %f, %f\n",  P.x(),  P.z(),  P.y() );
        ...
        {
            list<KeyFrameSP*>::reverse_iterator rit = keyframelist.rbegin();
            list<KeyFrameSP*>::reverse_iterator lrit;
        ...
        if (SHOW_L_EDGE)
        {
            if ((*it)->has_loop && (*it)->sequence == sequence_cnt)
            {
                ...
                KeyFrameSP* connected_KF = getKeyFrameSP((*it)->loop_index);

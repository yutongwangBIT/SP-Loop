    ...
    /*    vins_estimator::features features_for_pub;
        ...
        for (auto &it_per_id : estimator.f_manager.feature)
        {
            ...
            if(it_per_id.start_frame < WINDOW_SIZE - 2 && it_per_id.start_frame + frame_size - 1 >= WINDOW_SIZE - 2 && it_per_id.solve_flag == 1)
            {
                ...
                int imu_k = it_per_id.start_frame;
                Vector3d f_k = it_per_id.feature_per_frame[0].point * it_per_id.estimated_depth;
                Vector3d w_f_k = estimator.Rs[imu_k] * (estimator.ric[0] * f_k + estimator.tic[0])
                                        + estimator.Ps[imu_k];
                f_per_id.mWorldPos[0] = w_f_k(0);
                f_per_id.mWorldPos[1] = w_f_k(1);
                f_per_id.mWorldPos[2] = w_f_k(2);

                int imu_m = WINDOW_SIZE - 2 - it_per_id.start_frame;; //this should be the same as frame we need
                f_per_id.mPoint[0] = it_per_id.feature_per_frame[imu_m].point.x();
                f_per_id.mPoint[1] = it_per_id.feature_per_frame[imu_m].point.y();
                f_per_id.mUV[0] = it_per_id.feature_per_frame[imu_m].uv.x();
                f_per_id.mUV[1] = it_per_id.feature_per_frame[imu_m].uv.y();
            }
            else
                continue;
